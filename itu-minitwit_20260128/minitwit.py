...
app.config["MONGO_URI"] = (
    "mongodb://groupo:devopsgroupo@68.183.13.121:27017/minitwit?authSource=admin"
)
...
def user_timeline(username):
    ...
    messages = query_db(
        "message", {"author_id": str(profile_user["_id"])}, limit=PER_PAGE
    )

    followed = False
    if g.user:
        record = mongo.db.follower.find_one(
            {"who_id": str(g.user["_id"]), "whom_id": str(profile_user["_id"])}
        )
...
def follow_user(username):
    ...
    mongo.db.follower.insert_one(
        {"who_id": str(g.user["_id"]), "whom_id": str(whom_user["_id"])}
    )
